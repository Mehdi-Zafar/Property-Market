...
$prop_id_query = mysqli_query($con, "SELECT Prop_rent_id from wishlist_rent where User_id = '$id'");
                    ...
                    <div class="col">
                        <div class="card h-100 bg-gradient border-light shadow m-2" style="background-color: #FFFFCC;">
                            <div class="card-body">
                                ...
                                <p class="card-text"><b>Tenant Name: </b><?php $owner = $rows[$x - 1][0];
                                                                            $tenant_query = "SELECT Tenant_name from property_rent where Prop_rent_id = '$owner'";
                                                                            $result = mysqli_query($con, $tenant_query);
                                                                            while ($row2 = mysqli_fetch_array($result)) {
                                                                                $tenant_name = $row2[0];
                                                                            }
                                                                            echo "$tenant_name"; ?></p>
                                <p class="card-text"><b>Address: </b><?php $address_query = "SELECT Address from property_rent where Prop_rent_id = '$owner'";
                                                                        $result = mysqli_query($con, $address_query);
                                                                        while ($row2 = mysqli_fetch_array($result)) {
                                                                            $location = $row2[0];
                                                                        }
                                                                        echo "$location"; ?></p>
                                <p class="card-text"><b>Contact Number: </b><?php $contact_query = "SELECT Contact_number from property_rent where Prop_rent_id = '$owner'";
                                                                            $result = mysqli_query($con, $contact_query);
                                                                            while ($row2 = mysqli_fetch_array($result)) {
                                                                                $contact_number = $row2[0];
                                                                            }
                                                                            echo "$contact_number"; ?></p>
                                <p class="card-text"><b>House Rent(PKR): </b><?php $housevalue_query = "SELECT House_rent from property_rent where Prop_rent_id = '$owner'";
                                                                                $result = mysqli_query($con, $housevalue_query);
                                                                                while ($row2 = mysqli_fetch_array($result)) {
                                                                                    $value = $row2[0];
                                                                                }
                                                                                echo "$value"; ?></p>
                                <p class="card-text"><b>Status: </b><?php $Status_query = "SELECT Status from property_rent where Prop_rent_id = '$owner'";
