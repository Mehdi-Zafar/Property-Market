...
$ids = mysqli_query($con, "SELECT * from property_rent where Status = 'Unavailable'");
...
<body>
    ...
    <main>
        <ul class="nav nav-tabs justify-content-center mb-2">
            <li class="nav-item">
                <a class="nav-link" aria-current="page" href="Prop_rent.php">All houses</a>
            </li>
            <li class="nav-item">
                <a class="nav-link active" href="Prop_rented.php">Houses on rent</a>
            </li>
        </ul>
                ...
                <div class="col">
                    <div class="card h-100 bg-gradient border-light shadow m-2" style="background-color: #FFFFCC;">
                        <div class="card-body">
                            ...
                            <p class="card-text"><b>Tenant Name: </b><?php $tenant_query = "SELECT Tenant_name from property_rent where Status = 'Unavailable'";
                                                                        $result = mysqli_query($con, $tenant_query);
                                                                        while ($row2 = mysqli_fetch_all($result)) {
                                                                            $tenant_name = $row2[$x - 1][0];
                                                                        }
                                                                        echo "$tenant_name"; ?></p>
                            <p class="card-text"><b>Address: </b><?php $address_query = "SELECT Address from property_rent where Status = 'Unavailable'";
                                                                    $result = mysqli_query($con, $address_query);
                                                                    while ($row2 = mysqli_fetch_all($result)) {
                                                                        $location = $row2[$x - 1][0];
                                                                    }
                                                                    echo $location; ?></p>
                            <p class="card-text"><b>Contact Number: </b><?php $contact_query = "SELECT Contact_number from property_rent where Status = 'Unavailable'";
                                                                        $result = mysqli_query($con, $contact_query);
                                                                        while ($row2 = mysqli_fetch_all($result)) {
                                                                            $contact_number = $row2[$x - 1][0];
                                                                        }
                                                                        echo "$contact_number"; ?></p>
                            <p class="card-text"><b>House Rent(PKR): </b><?php $housevalue_query = "SELECT House_rent from property_rent where Status = 'Unavailable'";
