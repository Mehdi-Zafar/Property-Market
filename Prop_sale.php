...
$ids = mysqli_query($con, "SELECT * from property_sale");
...
<body>
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0 px-5">
                    ...
                    <li class="nav-item px-2">
                        <a class="nav-link active" href="Prop_sale.php">Property for Sale</a>
                    </li>
                    <li class="nav-item px-2">
    ...
    <main>
        <ul class="nav nav-tabs justify-content-center mb-2">
            <li class="nav-item">
                <a class="nav-link active" aria-current="page" href="Prop_sale.php">All houses</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="Prop_sold.php">Houses sold</a>
            </li>
        </ul>
        <div class="row row-cols-1 row-cols-md-3 g-4">
            <?php for ($x = 1; $x < $count + 1; $x++) : ?>
                <div class="col">
                    <div class="card h-100 bg-gradient border-light shadow m-2" id="card" style="background-color: #FFFFCC;">
                        <div class="card-body">
                            <h5 class="card-title text-center text-decoration-underline">Property <?php echo $x; ?></h5>
                            <p class="card-text"><b>Owner Name: </b><?php $owner_query = "SELECT Owner_name from property_sale where Prop_sale_id = '$x'";
                                                                    $result = mysqli_query($con, $owner_query);
                                                                    while ($row2 = mysqli_fetch_array($result)) {
                                                                        $owner_name = $row2[0];
                                                                    }
                                                                    echo "$owner_name"; ?></p>
                            <p class="card-text"><b>Address: </b><?php $address_query = "SELECT Address from property_sale where Prop_sale_id = '$x'";
                                                                    $result = mysqli_query($con, $address_query);
                                                                    while ($row2 = mysqli_fetch_array($result)) {
                                                                        $location = $row2[0];
                                                                    }
                                                                    echo "$location"; ?></p>
                            <p class="card-text"><b>Contact Number: </b><?php $contact_query = "SELECT Contact_number from property_sale where Prop_sale_id = '$x'";
                                                                        $result = mysqli_query($con, $contact_query);
                                                                        while ($row2 = mysqli_fetch_array($result)) {
                                                                            $contact_number = $row2[0];
                                                                        }
                                                                        echo "$contact_number"; ?></p>
                            <p class="card-text"><b>House Value(PKR): </b><?php $housevalue_query = "SELECT House_value from property_sale where Prop_sale_id = '$x'";
                                                                            $result = mysqli_query($con, $housevalue_query);
                                                                            while ($row2 = mysqli_fetch_array($result)) {
                                                                                $value = $row2[0];
                                                                            }
                                                                            echo "$value"; ?></p>
                            <p class="card-text"><b>Status: </b><?php $Status_query = "SELECT Status from property_sale where Prop_sale_id = '$x'";
                            ...
                            <?php if ($status == 'Available') : ?>
                                <form action="wishlist_sale.php" method="get">
