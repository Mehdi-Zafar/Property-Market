...
$ids = mysqli_query($con, "SELECT Address from property_sale where Status = 'Unavailable'");
$row = mysqli_fetch_all($ids);
...
<body>
    ...
    <main>
        <ul class="nav nav-tabs justify-content-center mb-2">
            <li class="nav-item">
                <a class="nav-link" aria-current="page" href="Prop_sale.php">All houses</a>
            </li>
            <li class="nav-item">
                <a class="nav-link active" href="Prop_sold.php">Houses sold</a>
            </li>
        </ul>
                ...
                <div class="col">
                    <div class="card h-100 bg-gradient border-light shadow m-2" style="background-color: #FFFFCC;">
                        <div class="card-body">
                            ...
                            <p class="card-text"><b>Owner Name: </b><?php $owner_query = "SELECT Owner_name from property_sale where Status='Unavailable'";
                                                                    $result = mysqli_query($con, $owner_query);
                                                                    while ($row2 = mysqli_fetch_all($result)) {
                                                                        $owner_name = $row2[$x - 1][0];
                                                                    }
                                                                    echo "$owner_name"; ?></p>
                            <p class="card-text"><b>Address: </b><?php $address_query = "SELECT Address from property_sale where Status='Unavailable'";
                                                                    $result = mysqli_query($con, $address_query);
                                                                    while ($row2 = mysqli_fetch_all($result)) {
                                                                        $location = $row2[$x - 1][0];
                                                                    }
                                                                    echo "$location"; ?></p>
                            <p class="card-text"><b>Contact Number: </b><?php $contact_query = "SELECT Contact_number from property_sale where Status='Unavailable'";
                                                                        $result = mysqli_query($con, $contact_query);
                                                                        while ($row2 = mysqli_fetch_all($result)) {
                                                                            $contact_number = $row2[$x - 1][0];
                                                                        }
                                                                        echo "$contact_number"; ?></p>
                            <p class="card-text"><b>House Value(PKR): </b><?php $housevalue_query = "SELECT House_value from property_sale where Status='Unavailable'";
                                                                            $result = mysqli_query($con, $housevalue_query);
                                                                            while ($row2 = mysqli_fetch_all($result)) {
                                                                                $value = $row2[$x - 1][0];
                                                                            }
                                                                            echo "$value"; ?></p>
                            <p class="card-text"><b>Status: </b>Unavailable</p>
                        </div>
                    </div>
                </div>
            <?php endfor; ?>
