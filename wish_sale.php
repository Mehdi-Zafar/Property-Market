...
// $ids = mysqli_query($con, "SELECT * from wishlist_sale where User_id = '$id'");
// $row = mysqli_fetch_array($ids, MYSQLI_ASSOC);
// $count = mysqli_num_rows($ids);
$prop_id_query = mysqli_query($con, "SELECT Prop_sale_id from wishlist_sale where User_id = '$id'");
$rows = mysqli_fetch_all($prop_id_query);
$count = mysqli_num_rows($prop_id_query);
?>
<!doctype html>
<html lang="en">
...
<body>
    ...
    <main>
        <div class="text-center text-decoration-underline m-2">
            <h1>Wishlist (For sale)</h1>
        </div>
        <?php if ($count > 0) : ?>
                    ...
                    <div class="col">
                        <div class="card h-100 bg-gradient border-light shadow m-2" style="background-color: #FFFFCC;">
                            <div class="card-body">
                                ...
                                <p class="card-text"><b>Owner Name: </b><?php $owner = $rows[$x - 1][0];
                                                                        $owner_query = "SELECT Owner_name from property_sale where Prop_sale_id = '$owner'";
                                                                        $result = mysqli_query($con, $owner_query);
                                                                        while ($row2 = mysqli_fetch_array($result)) {
                                                                            $owner_name = $row2[0];
                                                                        }
                                                                        echo "$owner_name"; ?></p>
                                <p class="card-text"><b>Address: </b><?php $address_query = "SELECT Address from property_sale where Prop_sale_id = '$owner'";
                                                                        $result = mysqli_query($con, $address_query);
                                                                        while ($row2 = mysqli_fetch_array($result)) {
                                                                            $location = $row2[0];
                                                                        }
                                                                        echo "$location"; ?></p>
                                <p class="card-text"><b>Contact Number: </b><?php $contact_query = "SELECT Contact_number from property_sale where Prop_sale_id = '$owner'";
                                                                            $result = mysqli_query($con, $contact_query);
                                                                            while ($row2 = mysqli_fetch_array($result)) {
                                                                                $contact_number = $row2[0];
                                                                            }
                                                                            echo "$contact_number"; ?></p>
                                <p class="card-text"><b>House Value(PKR): </b><?php $housevalue_query = "SELECT House_value from property_sale where Prop_sale_id = '$owner'";
                                                                                $result = mysqli_query($con, $housevalue_query);
                                                                                while ($row2 = mysqli_fetch_array($result)) {
                                                                                    $value = $row2[0];
                                                                                }
                                                                                echo "$value"; ?></p>
                                <p class="card-text"><b>Status: </b><?php $Status_query = "SELECT Status from property_sale where Prop_sale_id = '$owner'";
    ...
    <footer class="text-center text-lg-start bg-dark text-light pt-1 mt-5">
